...
class BoundingBoxTemplateRefiner:
    def __init__(self, image_path: str, template_dir: str = 'matching_templates'):
        ...
        self.template_classes = ['gate valve', 'reducer', 'check valve', 'strainer']
    ...
    def refine_template_bbox(self, bbox: Tuple[int, int, int, int], class_name: str) -> Tuple[int, int, int, int]:
        ...
        padding = max(max(t[1].shape) for t in self.templates[class_name]) // 2  # Max size of templates
        region = self.image[
        ...
        # Try all templates for this class
