...
| S5-04 | Define the multi-sheet merge contract for off-page connectors without implementing the full merge engine yet | V3 prep | `backend/schema/graph_v1.json`, `backend/schema/multi_sheet_merge_contract.md` | Schema/update note documents connector keys required for later merge work | ✅ DONE — `backend/schema/multi_sheet_merge_contract.md` defines merge key, direction inference, pairing rules, conflict types, and phased implementation plan. `graph_v1.json` edge schema extended with `off_page_connector` field. |
